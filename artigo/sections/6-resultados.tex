...
\begin{table}[htbp]
\centering
\caption{Tempos de render da etapa de verifica\c{c}\~ao (EEVEE, 64 amostras, $1500\times1000$)}
\label{tab:tempos}
\footnotesize
...
\bottomrule
\end{tabular}
\fonte{registro da sess\~ao de refer\^encia; a soma aritm\'etica das parcelas arredondadas
\'e 45,5\,s.}
\end{table}

O tempo de constru\c{c}\~ao completo da cena \'e da ordem de 0,6\,s --- cerca de setenta e
cinco vezes menor que o tempo total de render. Essa assimetria tem consequ\^encia
pr\'atica para o m\'etodo: \'e barato reconstruir a geometria e caro verific\'a-la, o que
justifica investir na verificabilidade autom\'atica das propriedades estruturais. A galeria de
apresenta\c{c}\~ao, executada a 96 amostras, consumiu aproximadamente 37\,s para as cinco
imagens.
...
Dois n\'umeros merecem leitura atenta. O primeiro \'e a contagem de n-gons: zero. Trata-se
de consequ\^encia direta das decis\~oes de arquitetura --- tampa por leque dobrado e
...
emblema, cujos glifos t\^em contorno aberto. Nenhum corpo curvo possui aresta de borda.

O Quadro~\ref{qua:origem-artefatos} decomp\~oe a origem dos tri\^angulos e das arestas de
borda. Nenhum tri\^angulo se encontra em superf\'icie curva principal.
...
\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{3.8cm}>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}p{8.8cm}}
\toprule
...
Demais 127 objetos & 2.456 & cantos de chanfro de tr\^es segmentos em primitivas \\
\midrule
...
quadrangular. O casco tem 750 quads e 752 v\'ertices; a cabine tem 148 quads em 150
v\'ertices. Ambos t\^em zero n-gons, zero arestas n\~ao variedade e zero arestas de borda
(Quadro~\ref{qua:malhas}).
...
Rodas (\texttt{\_Tire}, 4 objetos) & 2.880 & 2.880 & 2.880 & 0 \\
\bottomrule
\end{tabular}
\fonte{elaborado pelo autor a partir do relat\'orio de malha (2026).}
\end{quadro}
...
acrescentam 7 quads cada, alcan\c{c}ando 750 --- exatamente o valor medido. Do mesmo modo, a
cabine: 15 esta\c{c}\~oes geram 14 faixas de 10 faces (140 quads) e duas tampas de 4 quads
...
\label{tab:envelope}
\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{3.6cm}rrr>{\raggedright\arraybackslash}p{2.8cm}}
\toprule
...
ligeiramente al\'em do plano da \'ultima esta\c{c}\~ao. O valor permanece dentro da
...
\label{tab:grupos}
\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{5.2cm}r>{\raggedright\arraybackslash}p{6.4cm}}
\toprule
...
\begin{figure}[htbp]
  \centering
  \begin{tabular}{cc}
    \includegraphics[width=0.43\textwidth]{figures/ref-frontal.jpeg} &
    \includegraphics[width=0.43\textwidth]{figures/ver-frontal.png} \\
...
\subsubsection{Vista lateral}

A Figura~\ref{fig:cmp-lateral} \'e a vista mais informativa e a que exp\~oe a
limita\c{c}\~ao mais relevante do resultado. As propor\c{c}\~oes gerais --- altura da
cintura, posi\c{c}\~ao dos eixos, rela\c{c}\~ao entre o comprimento do cap\^o e o da
traseira, arco de roda proeminente --- s\~ao coerentes com a refer\^encia; o nariz baixo e a
cauda truncada est\~ao presentes.
...
refer\^encia, produz uma quebra deliberada nos reflexos ao longo de toda a extens\~ao. No
...
\begin{figure}[htbp]
  \centering
  \begin{tabular}{cc}
    \includegraphics[width=0.43\textwidth]{figures/ref-lateral.jpeg} &
    \includegraphics[width=0.43\textwidth]{figures/ver-lateral.png} \\
    \footnotesize refer\^encia & \footnotesize render
  \end{tabular}
...
\begin{figure}[htbp]
  \centering
  \begin{tabular}{cc}
    \includegraphics[width=0.43\textwidth]{figures/ref-traseira1.jpeg} &
    \includegraphics[width=0.43\textwidth]{figures/ver-traseira.png} \\
    \footnotesize refer\^encia & \footnotesize render
  \end{tabular}
...
seguran\c{c}a. O crit\'erio CA-008 \'e atendido.

\begin{figure}[htbp]
  \centering
  \begin{tabular}{cc}
    \includegraphics[width=0.43\textwidth]{figures/ref-superior.jpeg} &
    \includegraphics[width=0.43\textwidth]{figures/ver-superior.png} \\
    \footnotesize refer\^encia & \footnotesize render
  \end{tabular}
...
\subsubsection{Vista em tr\^es-quartos e pintura}

A Figura~\ref{fig:cmp-3q} confronta o render em tr\^es-quartos com a refer\^encia azul, que
\'e a imagem de mesma cor do material. Os reflexos s\~ao cont\'inuos ao longo do para-lama
...
Duas causas concorrem para o desvio de tom. A exposi\c{c}\~ao global foi deliberadamente
reduzida ($-0{,}18$) para preservar as altas luzes nos reflexos especulares; e a
refer\^encia fotogr\'afica passou pelo mapeamento de tons da c\^amera, diferente do AgX.
Isolar a contribui\c{c}\~ao de cada causa exigiria um experimento controlado com um alvo de
cor f\'isico, que n\~ao foi realizado.

\begin{figure}[htbp]
  \centering
  \begin{tabular}{cc}
    \includegraphics[width=0.43\textwidth]{figures/ref-car.jpeg} &
    \includegraphics[width=0.43\textwidth]{figures/ver-3q.png} \\
    \footnotesize refer\^encia & \footnotesize render
  \end{tabular}
...
\subsubsection{Detalhe da roda e vistas em material neutro}
...
requisito que define a identidade da variante pelas rodas de seis raios \'e atendido, e o
pneu apresenta o perfil e a propor\c{c}\~ao derivados da designa\c{c}\~ao 185/55R15.

A Figura~\ref{fig:clay} apresenta as duas vistas em material neutro, cuja fun\c{c}\~ao \'e
remover a vari\'avel da cor e do verniz e permitir a leitura da forma. A vista lateral em
material neutro \'e a evid\^encia mais clara da limita\c{c}\~ao de silhueta: sem o verniz,
a superf\'icie \'e uniformemente suave, e a aus\^encia da aresta viva ao longo do para-lama
torna-se evidente --- ao contr\'ario do que ocorreria se o reflexo do verniz mascarasse a
leitura.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.62\textwidth]{figures/ver-roda.png}
...
\begin{figure}[htbp]
  \centering
  \begin{tabular}{cc}
    \includegraphics[width=0.43\textwidth]{figures/ver-clay-lateral.png} &
    \includegraphics[width=0.43\textwidth]{figures/ver-clay-3q.png} \\
...
\label{qua:aceite}
\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{1.3cm}>{\raggedright\arraybackslash}p{5.4cm}>{\raggedright\arraybackslash}p{1.7cm}>{\raggedright\arraybackslash}p{5.4cm}}
\toprule
\textbf{ID} & \textbf{Crit\'erio} & \textbf{Status} & \textbf{Evid\^encia} \\
\midrule
CA-001 & \emph{Build} sem erro & PASS & 130 objetos \texttt{GEO\_*} \\
...
variedade em toda a cena. Segunda: o envelope dimensional atende \`a toler\^ancia, com
desvio m\'aximo de $1{,}0$\,cm. Terceira: os elementos de identidade da variante --- rodas
de seis raios, conjunto \'optico, entradas de ar com geometria real, emblemas --- est\~ao
presentes e s\~ao verific\'aveis nos renders. Quarta: a fidelidade de silhueta e de tom \'e
parcial, e as causas s\~ao identificadas e localizadas.

A aus\^encia de um controle experimental --- um segundo modelo constru\'ido pelo m\'etodo
manual --- impede que se afirme, com base em medi\c{c}\~ao, que este \emph{pipeline} seja
melhor que o tradicional. O que se pode afirmar \'e que o artefato \'e reconstru\'ivel por
um comando, que suas propriedades estruturais s\~ao medidas automaticamente e que cada
decis\~ao geom\'etrica \'e rastre\'avel at\'e uma linha de c\'odigo.
